  ...
  petList.forEach((pet) => {
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        <img src="delete.png" type="button" onclick="deletePet('${pet.id}')"/>
      </td>
    `;
...
function previewImage(event) {
  ...
  if (file) {
    ...
    reader.onload = function (e) {
      imgInput.value = e.target.result; // Lưu URL base64 vào input (nếu cần)
      const imgPreview = document.getElementById("img-preview-1"); // Thay id của thẻ img preview
      if (imgPreview) {
        imgPreview.src = e.target.result; // Hiển thị ảnh
      } else {
...
// Load ảnh từ localStorage nếu đã lưu trước đó
document.addEventListener("DOMContentLoaded", () => {
  const savedImage = localStorage.getItem("uploadedImage");
  if (savedImage) {
    const imgPreview = document.getElementById("img-preview-1");
    if (imgPreview) {
      imgPreview.src = savedImage; // Hiển thị ảnh đã lưu
    }
  }
});
